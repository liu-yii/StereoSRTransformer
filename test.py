...
def batched_predict(model, inp_left, inp_right, coord, cell, bsize):
    with torch.no_grad():
        ...
        while ql < n:
            qr = min(ql + bsize, n)
            pred_left, disp_r2l = model.query_rgb_left(coord[:, ql: qr, :], cell[:, ql: qr, :])
            pred_right, disp_l2r = model.query_rgb_right(coord[:, ql: qr, :], cell[:, ql: qr, :])
...
def eval_psnr(loader, model, save_dir, data_norm=None, eval_type=None, eval_bsize=None, scale_max=4, fast=False,
    ...
    for batch in pbar:
        ...
        if eval_bsize is None:
            with torch.no_grad():
                preds_left, preds_right = model(inp_left, inp_right, coord, cell)
        else:
            if fast:
                preds_left, preds_right = model(inp_left, inp_right, coord, cell*max(scale/scale_max, 1))
            else:
