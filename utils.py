...
# def warp_disp(img, disp, img_size=None):
#     '''
...
def warp(img, disp, img_size=None, mode='l2r'):
    ...
    # Original coordinates of pixels
    x_base = torch.arange(0, w).repeat(b, h, 1).type_as(img)
    y_base = torch.arange(0, h).repeat(b, w, 1).transpose(1, 2).type_as(img)
...
def warp_coord(coord, disp, raw_hr, hr_size, mode='l2r'):
    '''
    Borrowed from:
    '''
    b, _, _ = coord.shape
    ...
    raw_hr = raw_hr.permute(0,2,1).view(-1, 3, hr_size[0][0], hr_size[1][0])
