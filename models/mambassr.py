...
#         xs = torch.cat([x_hwwh, torch.flip(x_hwwh, dims=[-1])], dim=1) # (1, 4, 192, 3136) # B,4,C,L
...
#         xz = self.in_proj(x)    # B,H,W,4C
#         x, z = xz.chunk(2, dim=-1)   # B,H,W,2C  ; B,H,W,2C

#         x = x.permute(0, 3, 1, 2).contiguous()   # B,2C,H,W
#         x = self.act(self.conv2d(x))  # B,2C,H,W
#         y1, y2, y3, y4 = self.forward_core(x)   #2d-SSM, y1,y2,y3,y4分别是不同的方向的序列
...
#     def forward_features(self, x, x2, cost):
#         x_size = (x.shape[2], x.shape[3])
#         x = self.patch_embed(x) # N,L,C
#         x2 = self.patch_embed(x2) # N,L,C

#         x = self.pos_drop(x)
#         x2 = self.pos_drop(x2)

#         for layer in self.layers:
#             x, x2, cost = layer(x,x2, x_size,cost)

#         x = self.norm(x)  # b seq_len c
#         x = self.patch_unembed(x, x_size)

#         x2 = self.norm(x2)  # b seq_len c
#         x2 = self.patch_unembed(x2, x_size)

#         return x,x2,cost

#     def forward(self, left, right, cost):
#         self.mean = self.mean.type_as(left)
#         left = (left - self.mean) * self.img_range
#         right = (right - self.mean) * self.img_range
#         feat_left = self.conv_first(left)
#         feat_right = self.conv_first(right)
#         shallow_feat_l, shallow_feat_r = feat_left, feat_right
        

        
#         feat_left, feat_right, cost = self.forward_features(feat_left, feat_right, cost)
        
#         feat_left = self.conv_after_body(feat_left) + shallow_feat_l
#         feat_right = self.conv_after_body(feat_right) + shallow_feat_r
...
#         self.fusion = SCAM(dim).cuda() if is_fusion else None
...
#     def forward(self, x, x2, x_size, cost):
        
#         feat_left = self.conv(self.patch_unembed(self.residual_group(x, x_size), x_size))
#         feat_right = self.conv(self.patch_unembed(self.residual_group(x2, x_size), x_size))
#         if self.is_fusion:
#             feat_left, feat_right, cost = self.fusion(feat_left, feat_right, cost)
#         out = self.patch_embed(feat_left) + x
#         out2 = self.patch_embed(feat_right) + x2
#         return out, out2, cost
...
# @register('mambassr')
# def make_mambassr(up_scale=4, width=48, num_blks=16, img_channel=3, drop_path_rate=0., drop_out_rate=0., fusion_from=-1, fusion_to=16, dual=True, no_upsampling =True):
#     args = Namespace()
...
#     return MambaSSR(img_size=48,
#                  patch_size=1,
#                  in_chans=args.img_channel,
#                  embed_dim=96,
#                  depths=(6, 6, 6, 6),
#                  drop_rate=0.,
#                  d_state = 16,
